...
import { getLeaderboardData, getHubStats, listFeaturedSkills, listProjectOptions } from "@/lib/store";
...
export default async function Home({ searchParams }: HomeProps) {
  ...
  const [featured, stats, leaderboard] = await Promise.all([
    listFeaturedSkills(3, selectedProject),
    getHubStats(selectedProject),
    getLeaderboardData(3, selectedProject),
  ]);

  return (
    <AppShell>
      <section className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr] lg:items-stretch">
        <div className="section-card flex h-full flex-col justify-between lg:p-7">
          <div className="inline-flex w-fit items-center gap-2 rounded-full bg-sky-50 px-3 py-1 text-xs font-semibold text-sky-700">
            <Sparkles className="h-4 w-4" />
            {pick(locale, "在线发布 · 超级管理员审批 · Registry 安装闭环", "Online publishing · admin review · registry-ready installation")}
          </div>
          <h1 className="mt-4 max-w-4xl text-3xl font-semibold tracking-tight text-slate-950 md:text-4xl md:leading-[1.1]">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <Link
              href="/publish"
              className="button-primary h-10 gap-2 px-4 text-xs"
            >
          ...
          <form method="get" className="mt-4 flex max-w-md gap-2">
            <select name="project" defaultValue={selectedProject} className="field-input h-10 flex-1">
              <option value="">全部项目</option>
              {projects.map((project) => (
                <option key={project.id} value={project.id}>{project.name}</option>
              ))}
            </select>
            <button type="submit" className="button-secondary h-10 px-4 text-xs">筛选</button>
          </form>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            {[
              ...
              <div key={item.label} className="rounded-2xl border border-slate-100 bg-slate-50/90 px-4 py-3">
        ...
        <div className="surface-card p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">{pick(locale, "收藏热度榜", "Most Favorited")}</h2>
            </div>
            <Flame className="h-5 w-5 text-rose-500" />
          </div>
          <div className="mt-4 space-y-2">
            {leaderboard.favorites.map((item, index) => (
              <Link key={item.slug} href={`/skills/${item.slug}`} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 transition hover:bg-white hover:shadow-sm">
                <div>
                  <div className="text-sm font-semibold text-slate-950">#{index + 1} {item.displayName}</div>
        ...
        <div className="surface-card p-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-2 text-xl font-semibold tracking-tight text-slate-950">{pick(locale, "评分口碑榜", "Top Rated")}</h2>
            </div>
            <Star className="h-5 w-5 text-amber-500" />
          </div>
          <div className="mt-4 space-y-2">
            {leaderboard.ratings.map((item, index) => (
              <Link key={`${item.slug}-rating`} href={`/skills/${item.slug}`} className="flex items-center justify-between rounded-2xl bg-slate-50 px-4 py-3 transition hover:bg-white hover:shadow-sm">
                <div>
                  <div className="text-sm font-semibold text-slate-950">#{index + 1} {item.displayName}</div>
